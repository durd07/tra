...
func recvNotification(stream tra.TraService_SubscribeClient) {
	for {
...
func main() {
	...
	defer conn.Close()
	c := tra.NewTraServiceClient(conn)

	ctx, cancel := context.WithTimeout(context.Background(), time.Second*100000)
	defer cancel()

	stream, err := c.Subscribe(ctx, &tra.TraRequest{})
	if err != nil {
